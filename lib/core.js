  ...
  ReactChartJS.createClass = function(chartType, methodNames, dataKey) {
    ...
    classData.componentWillReceiveProps = function(nextProps) {
      ...
      if (nextProps.redraw) {
        ...
        ReactChartJS.updatePoints(nextProps, chart, dataKey);
  ...
  if (name === 'PolarArea' || name === 'Pie' || name === 'Doughnut') {
    nextProps.data.forEach(function(segment, segmentIndex) {
      if (!chart.segments[segmentIndex]) {
        ReactChartJS.chart.addData(segment);
      } else {
    ...
    nextProps.data.datasets.forEach(function(set, setIndex) {
      set.data.forEach(function(val, pointIndex) {
        if (typeof(chart.datasets[setIndex][dataKey][pointIndex]) == "undefined") {
          ReactChartJS.addData(nextProps, chart, setIndex, pointIndex);
        } else {
...
ReactChartJS.Doughnut = ReactChartJS.createClass('Doughnut', ['getSegmentsAtEvent']);
ReactChartJS.Line = ReactChartJS.createClass('Line', ['getPointsAtEvent']);
ReactChartJS.Pie = ReactChartJS.createClass('Pie', ['getSegmentsAtEvent']);
ReactChartJS.PolarArea = ReactChartJS.createClass('PolarArea', ['getSegmentsAtEvent']);
ReactChartJS.Radar = ReactChartJS.createClass('Radar', ['getPointsAtEvent']);